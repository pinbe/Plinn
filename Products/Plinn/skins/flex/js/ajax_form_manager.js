...
(function() {
    var raiseMouseEvent = function(ob, eventName) {
    ...
    FormManager = function(form, responseTextDest, lazy, noHistory) {
        ...
        if(this.lazy) {
            this.form.onclick = function(evt) {
                self.replaceElementByField(evt);
                self.click(evt);
            };
            this.form.onfocus = this.form.onclick;
            this.onResponseLoad = function(req) {
                self.restoreField(req);
            };
            this.lazyListeners = [];
        }
    };
            ...
            switch(ob.tagName) {
                case 'SPAN' :
                    ...
                    this.lazyListeners.push({
                        'element': inputText, 'eventName': 'blur', 'handler': function() {
                            self.submit();
                        }
                    });
                    this.lazyListeners.push({
                        'element': inputText, 'eventName': 'keypress', 'handler': function(evt) {
                            self._fitField(evt);
                        }
                    });
                ...
                case 'P' :
                    ...
                    this.lazyListeners.push({
                        'element': ta, 'eventName': 'blur', 'handler': function() {
                            self.submit();
                        }
                    });
                    this._addLazyListeners();
